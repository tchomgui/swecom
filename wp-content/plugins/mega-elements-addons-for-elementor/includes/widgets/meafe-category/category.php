...
class MEAFE_Category extends Widget_Base
{
        ...
        foreach ( $cats as $key => $value ) 
        {
            ...
            if( $category )
            {
                ...
                if( $settings['bccls_category_show_image'] && isset( $img ) && is_array( $img ) && isset( $img[0] ) && $img[0] !='' )
                {
                    ...
                    echo '<a '.$target.' href="'. esc_url( get_category_link( $value ) ) .'"><img src="' . esc_url( $url1 ) . '" alt="'. get_cat_name( $value ) .'"><div class="title-wrapper"><span class="meafe-entry-title">'.get_cat_name( $value ).'</span>';
                ...
                else
                {
                    ...
                    echo '<a '.$target.' href="'. esc_url( get_category_link( $value ) ) .'">' . $url1 . '<div class="title-wrapper"><span class="meafe-entry-title">'.get_cat_name( $value ).'</span>';
                    if( $settings['bccls_category_show_meta'] && $count > 0 ) {
                        echo '<span class="meafe-entry-meta">'.esc_html( $count ).__(' Post(s)','mega-elements-addons-for-elementor').'</span>';
                    }
                    echo '</div></a></li>';
    ...
    protected function render()
    {
        ...
        echo '<div ' . $this->get_render_attribute_string( 'category_wrapper' ) . '>
            <div class="meafe-post-grid meafe-post-appender meafe-post-appender-' . $this->get_id() . '">
